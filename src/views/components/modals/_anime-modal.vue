<template lang="pug">
  ...
  transition(name='modal-fade')
    ...
    div(class='modal-backdrop modal-backdrop--anime')

      div(class='modal modal--anime')
          ...
          div(class='modal__fields')
            ...
            app-dropdown(
              ...
              li(
                ...
                v-for='status in anime.statuses'
...
export default {
  ...
  methods: {
    closeModal() { 
      eventBus.$emit('closeModal', 'anime') 
    },
    ...
    setDefaults() {
      let animeData = this.$store.state["anime"]
      ...
      if (this.current.status.length == 0) {
        let defaultStatus = animeData.statuses.filter(i => i.default)[0]
    ...
    slotExist(id) {
      if (this.animeCollection.some(e => e.id == id)) {
        return true
      }
      return false
    },
